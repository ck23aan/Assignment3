...
def worldbank_data(filename):
    ...
    Returns:
    - df_byyears (DataFrame): Dataframe with rows representing
      years and columns representing countries.
    - df_bycountry (DataFrame): Original dataframe representing
...
def inverse_transform_data(normalized_df, scaler, features):
    """
    Function to inverse transform the normalized data back to
    the original scale.
    ...
    """
    inverse_transformed_data = scaler.inverse_transform(
...
df_byyears, df_bycountry = worldbank_data(
    'API_NY.GDP.PCAP.CD_DS2_en_csv_v2_6298251.csv')
df = df_bycountry[
    ['Country Name', 'Indicator Name'] + list(map(str, range(1960, 2023)))]
df = df.dropna()
...
scatter_plot = plt.scatter(
    dfx["1960"], dfx["Growth"], 10, label="GDP per capita (current USD)")
...
centroids_inv = inverse_transform_data(
    pd.DataFrame(
        centroids, columns=["1960", "Growth"]), scaler, ["1960", "Growth"])
xkmeans, ykmeans = centroids_inv["1960"], centroids_inv["Growth"]

plt.figure(figsize=(7, 5))
plt.scatter(dfx["1960"], dfx["Growth"], c=labels, marker="o", s=10)
plt.scatter(
    xkmeans, ykmeans, marker="x", c="red", s=50, label="Cluster Centroids")
plt.xlabel("GDP per capita (current USD) in 1960")
plt.ylabel("Growth Percentage in GDP per capita from 1960 to 2022")
...
# ## Curve Fit
world_GDP = df_byyears.loc['1960':'2022', ['World']].reset_index().rename(
    columns={'index': 'Year', 'World': 'GDP per capita'})
...
param, covar = opt.curve_fit(
    exponential, world_GDP["Year"],
    world_GDP["GDP per capita"], p0=(1.3e4, 0.1))
world_GDP["fit"] = exponential(world_GDP["Year"], *param)
plt.figure(figsize=(7, 5))
sns.lineplot(
    data=world_GDP, x="Year", y="GDP per capita",
...
sns.lineplot(
    x= world_GDP["Year"], y= world_GDP["GDP per capita"],
    label="World GDP per capita")
sns.lineplot(x=years, y=predictions, label="Prediction", color='red')
plt.fill_between(
    years, predictions - confidence_range, predictions + confidence_range,
